...
export default function ParagraphsPage() {
  ...
  const handleAudioSubmit = async (file: File, targetText: string) => {
    setIsLoading(true);
    setError(null);
    setData(null);

    try {
      const result = await analyzeAudio(file, targetText);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <p className="text-muted-foreground font-medium flex items-center gap-2">
                  ...
                  Read full sentences or short paragraphs to analyze continuous speech patterns
                </p>
              </div>
            </div>
